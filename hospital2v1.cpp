...
+}
+
+// Função para exibir os dados de um cadastro
+template <typename T>
+void exibirDados(const std::vector<T>& dados) {
...
+
+int main() {
+    // Inicializando os vetores de cadastros
+    std::vector<Cidades> cidades;
+    std::vector<Especialidades> especialidades;
+    std::vector<Medicos> medicos;
+    std::vector<Pacientes> pacientes;
...
+    std::vector<Consultas> consultas;
+
+    // Preenchendo os vetores de cadastros
+    // Implemente a leitura dos dados e a adição ao vetor correspondente
+
+    // Menu de opções
+    int opcao;
+    do {
+        std::cout << "Menu de Opções:" << std::endl;
+        std::cout << "1. Cidades" << std::endl;
+        std::cout << "2. Especialidades" << std::endl;
+        std::cout << "3. Médicos" << std::endl;
+        std::cout << "4. Pacientes" << std::endl;
+        std::cout << "5. CID" << std::endl;
+        std::cout << "6. Medicamentos" << std::endl;
+        std::cout << "7. Consultas" << std::endl;
...
+                break;
+            case 2:
+                exibirDados(especialidades);
+                break;
+            case 3:
+                exibirDados(medicos);
+                break;
+            case 4:
+                exibirDados(pacientes);
+                break;
+            case 5:
+                exibirDados(cids);
+                break;
+            case 6:
+                exibirDados(medicamentos);
+                break;
+            case 7:
+                exibirDados(consultas);
+                break;
+            case 0:
+                break;
+            default:
...
+    return 0;
+}
<<<<<<<  8d21b289-8549-4262-98fb-9529c2a0de47  >>>>>>>