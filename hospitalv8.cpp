...
struct Consultas {
    ...
    double custo_medicamento; // Add this line
};
...
Pacientes lerPaciente() {
    ...
    std::getline(std::cin, paciente.CPF);
...
Consultas lerConsulta() {
    ...
    std::cin.ignore();
    std::cout << "Digite o CPF do paciente: ";
    std::getline(std::cin, consulta.CPF);
...
void agendarConsulta(std::vector<Pacientes>& pacientes, std::vector<Medicos>& medicos, std::vector<CID>& doencas,
    ...
    std::string codigo_paciente;
    std::cin.clear();
    std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
    std::cout << "Digite o CPF do paciente: ";
    std::getline(std::cin, codigo_paciente);
    ...
    // Calcular o custo do medicamento utilizado na consulta
    double custo_medicamento = it_medicamento->preco_unitario * qtde_medicamento;

    // Criar um novo registro de consulta e adicionar ao vetor de consultas
    Consultas nova_consulta{ codigo_paciente, codigo_medico, data_consulta, "", codigo_CID, codigo_medicamento, qtde_medicamento, custo_medicamento };
...
void sugestaoDeCompras(std::vector<Medicamentos>& medicamentos) {
    for (const auto& medicine : medicamentos) {
        if (medicine.quant_estoque <= medicine.estoque_maximo * 0.1) { // Check if the stock is less than or equal to 10% of the minimum stock
            std::cout << "Codigo: " << medicine.codigo << ", Descricao: " << medicine.descricao
                      ...
                      << ", Sugestao de compra: " << medicine.estoque_maximo - medicine.quant_estoque << std::endl;
        }
    }
}
...
void valorTotalArrecadado(std::vector<Consultas>& consultas, std::vector<Medicamentos>& medicamentos) {
    ...
    for (const auto& consulta : consultas) {
        // Subtrair o custo do medicamento do valor total da consulta
        double valorConsulta = 100.0 + consulta.custo_medicamento;
        ...
        switch (opcao) {
            ...
            case 3:
                ...
                switch (opcaoControle) {
                    case 1:
                        sugestaoDeCompras(medicines);
                        break;
                    case 2:
                        valorTotalArrecadado(consultas, medicines);
                        break;
                    default:
                        std::cout << "Opcao invalida!\n";
                        break;
